	...
	for(auto o : nearby_animals)
	{
		...
		if(Animal* a = dynamic_cast<Animal*>(o)) {
			...
			for(auto& c : cases) {
				...
				w_type += (!i_am_herbivore && !a->is_alive()) ? 1000.0 : 0.0; // carnivores automatically considers cases with dead animals
...
int CBR::reason_carnivore(vector<Case> cases)
{
	...
	float highest_weight = (me->get_action() != ATTACK) ? 0.0 : 0.55; // if we are already attacking something, it will take a pretty high value to change state
